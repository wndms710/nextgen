#ifndef INFLOWTURBULENCEH
#define INFLOWTURBULENCEH
#include "bcprofile/ProfileReader.hpp"

void InflowTurbulenceHBc::setStatistics() {
  ...
  while (iarg < p->size()) {
    ...
    if (token == "PROFILE") {
      ...
      //    PROFILE HOOK
      if (token == "HOOK") {
        solver->inflowTurbulenceStatsHook(um_it, Rd_it,Rod_it,zone_ptr);
      }//"HOOK"

      //    constant properties across the zone given tks
      //    PROFILE CONSTANT_UTKE <ux> <uy> <uz> <tke>
      //    NOTE: turbulent kinetic enery (tke) is defined as:
      //          tke = 1/2 * (u'u'+v'v'+w'w')
      else if ( token == "CONSTANT_UTKE" ) {
        double my_um_it[3];
        my_um_it[0] = p->getDouble(iarg++);
        my_um_it[1] = p->getDouble(iarg++);
        my_um_it[2] = p->getDouble(iarg++);
        double tke = p->getDouble(iarg++);
        if (mpi_rank == 0)
          cout << "  > using constant mean velocity, constant turbulent kinetic energy: (ux,uy,uz) = (" << my_um_it[0] << "," << my_um_it[1] << "," << my_um_it[2] << "), tke = " << tke << endl;
        double ms  = 2.0/3.0 * tke;
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          FOR_I3 um_it[ibf][i]  = my_um_it[i];
          FOR_I3 Rd_it[ibf][i]  = ms;
          FOR_I3 Rod_it[ibf][i] = 0.0;
        }//(ibf < nbf)
      }//"CONTSTANT UTKE"
      else if (token == "TRIP_CONSTANT_TKE") {
        double tke = p->getDouble(iarg++) ;
        if (mpi_rank == 0 ) {
      ...
      //    PROFILE CONSTANT_UMS <ux> <uy> <uz> <u'u'> <v'v'> <w'w'>
      else if ( token == "CONSTANT_UMS" ) {
        double my_um_it[3];
        my_um_it[0] = p->getDouble(iarg++);
        my_um_it[1] = p->getDouble(iarg++);
        my_um_it[2] = p->getDouble(iarg++);
        ...
        if (mpi_rank == 0)
          cout << "  > using constant mean velocity and constant turbulent mean squares: (ux,uy,uz) = (" <<
            my_um_it[0] << "," << my_um_it[1] << "," << my_um_it[2] << "), (uxux,uyuy,uzuz) = (" <<
            my_uu_bc[0] << "," << my_uu_bc[1] << "," << my_uu_bc[2] << ")" << endl;
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          FOR_I3 um_it[ibf][i]  = my_um_it[i];
      ...
      //    PROFILE CONSTANT_UREY <ux> <uy> <uz> <u'u'> <v'v'> <w'w'> <v'w'> <u'w'> <u'v'>
      //    and
      //    Rd  = [<u'u'> <v'v'> <w'w'>]
      //    Rod = [<v'w'> <u'w'> <u'v'>]
      else if ( token == "CONSTANT_UREY" ) {
        double my_um_it[3];
        my_um_it[0] = p->getDouble(iarg++);
        my_um_it[1] = p->getDouble(iarg++);
        my_um_it[2] = p->getDouble(iarg++);
        ...
        if (mpi_rank == 0)
          cout << "  > using constant mean velocity and constant Reynolds stresses: (ux,uy,uz) = (" <<
            ...
            my_uv_bc[0] << "," << my_uv_bc[1] << "," << my_uv_bc[2] << ")" << endl;
        for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
          FOR_I3 um_it[ibf][i]  = my_um_it[i];
          FOR_I3 Rd_it[ibf][i]  = my_uu_bc[i];
      ...
      //            tke = 1/2 * (u'u'+v'v'+w'w')
        ...
        COUT2("  > using profiles of velocity and tke from Fluent RANS file: " << token);
        //      read the profile
      ...
      //    PROFILE FLUENT_UREY
        ...
        COUT2("  > using profiles of velocity and Reynolds stresses from Fluent RANS file: " << token);
        //      read the profile
        FluentBPReader profile(token);
        //      set the points
        //      old comment:
    ...
    else if(token=="L_RATIO"){
      token=p->getString(iarg++);
      length_ratio = p->getDouble(iarg-1); //out/in
      COUT2("  > Ratio of out of plane lengthscale to in plane length scale " << length_ratio );
    }//"L_RATIO"
    else if ( token == "UN") {
      Un = p->getDouble(iarg++) ;
    ...
    else if (token=="MODIFY_U"){
      b_modify_u = true;
      COUT2(" > Applying inflow turbulence after solveU");
    }
    else {
...
void InflowTurbulenceHBc::updateBC(double (*FacePtrNew)[3],double (*FacePtrOld)[3]) {
  ...
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
    double length=length_scale[ibf]*length_ratio;
    double Tper;
...
void InflowTurbulenceHBc::query(const string& param_str) const {
  ...
  for (int ibf = 0; ibf < zone_ptr->nbf; ++ibf) {
    ...
    if (zone_ptr->area_bf[ibf] > 0.0) {
      ...
      double n_bf_mag = MAG(zone_ptr->n_bf[ibf]);
      const double un = -DOT_PRODUCT(up_it[ibf],zone_ptr->n_bf[ibf])/n_bf_mag;
  ...
  if ( mpi_rank == 0 ) {
    ...
    *ss << " > " << std::left << std::setw(30) << zone_ptr->getName() << " :  time, un_avg, un_rms, p_cv_avg, p_cv_rms = " << std::right
        << std::setw(8) << solver->time << " " << std::setw(12) << un_avg << " "
        << std::setw(12) << sqrt(max(0.0,buf[2]/buf[0]-un_avg*un_avg)) << " " << std::setw(12) << p_avg << " " << std::setw(12) << sqrt(max(0.0,buf[4]/buf[0]-p_avg*p_avg)) << endl;
  }
  flush();

}

int InflowTurbulenceHBc::solveVecBoundaryCvCg(double (*sol)[3],const double * const A,double (*rhs)[3], double * AddedDiag,double zero, int maxiter ) {
  bool verbose = false;
...
//            REDUNDANT CODE
///////////////////////////////////////////////////////////////////////////////////
void InflowTurbulenceHBc::updateInflowTurbulence(double (*FacePtr)[3]){
  ...
  for (int ibf=0; ibf<zone_ptr->nbf; ++ibf) FOR_I3 RandomField[ibf][i] = 0.0;
  ...
  //  update the boundary conditions. The turbulent field is stored in up_it, u_bc stores full field
  updateBC(RandomField,FacePtr);
  delete [] RandomField;
}//updateInflowTurbulence()

void InflowTurbulenceHBc::buildITBoundaryStructures() {
  ...
  for(vector<HelmholtzBc*>::iterator it = solver->bcs.begin(); it != solver->bcs.end(); ++it){
    string s=(*it)->getName();
    Param* p=getParam(s);
    //    cout<<p->getString()<<endl;
    if(p->getString()=="WALL"||p->getString()=="WM_SLIP_WALL"||p->getString()=="WM_ALG_WALL"){
...
void InflowTurbulenceHBc::setLengthscale() {
  ...
  if(in_plane_length==1.0E+200){ // A constant lengthscale is not specified and we calculate the distance to the wall with a maximum threshold L_MAX
    ...
    //    First we compute the distance to the wall in these cvs
    for(vector<HelmholtzBc*>::iterator it = solver->bcs.begin(); it != solver->bcs.end(); ++it){
      string s=(*it)->getName();
      Param* p=getParam(s);
      if(p->getString()=="WALL"||p->getString()=="WM_SLIP_WALL"||p->getString()=="WM_ALG_WALL"){
  ...
  else solver->inflowTurbulenceLengthscaleHook(length_scale,zone_ptr);
}//setLengthScale()

void InflowTurbulenceHBc::buildFilterOperators() {
...
void InflowTurbulenceHBc::initiateStats() { // this must be smart enough to recognize a restart
  if (!zone_ptr->checkDataFlag("up_it")) {  // if the velocity isn't read in it must be set to 0 and the statistics should be reset
    reset=true;
    for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
      FOR_I3 up_it[ibf][i]=0.0;
    }//(ibf < zone_ptr->nbf)
  }//("up_it")
...
void InflowTurbulenceHBc::updateRandomField(double (*FacePtr)[3]) {
...
void InflowTurbulenceHBc::removeMeanAndSetVarianceOne(double (*FacePtr)[3]) {
...
void InflowTurbulenceHBc::computeCholesky(double (*FacePtr)[3]) {
  ...
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
    ...
    //    velocity component
    double my_up_it[3];
    my_up_it[0]  = a11 * FacePtr[ibf][0];
    my_up_it[1]  = a21 * FacePtr[ibf][0] + a22 * FacePtr[ibf][1];
    my_up_it[2]  = a31 * FacePtr[ibf][0] + a32 * FacePtr[ibf][1] + a33 * FacePtr[ibf][2];

    FOR_I3 FacePtr[ibf][i]=my_up_it[i];
  }//(ibf < nbf)
}//computeCholesky()

void InflowTurbulenceHBc::updateCvData_(double (*var_)[3]) {
...
void InflowTurbulenceHBc::updateCvData_(double *var_) {
...
void InflowTurbulenceHBc::getCvData_(double *FacePtr, double *CvPtr_){
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
...
//two pointers as input. Moves data from the cv pointer to the right face.
void InflowTurbulenceHBc::getCvVectorData_(double (*FacePtr)[3], double (*CvPtr)[3]){
  for (int ibf =0; ibf < zone_ptr->nbf; ++ibf) {
    FOR_I3{
