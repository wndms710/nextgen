...
//#define ADT2D_LIST_MAX 8192
// TODO XXXXXX: get rid of this eventually by using the "vector<int> bboxVec" calls...
#define ADT2D_LIST_MAX 1000000

using namespace std ; 

template <class T>
class Adt2dIndexCompare : public std::binary_function<T, T, bool> {
 private:
  const T (*bb)[2];
  int id;                
 public: 
  Adt2dIndexCompare(const T (*bb)[2],int id) {
    this->bb = bb;
    this->id = id;
  } 
...
protected:
 ...
 public:
  ...
  Adt2d(int n,const T (*bbmin)[2],const T (*bbmax)[2]) {

    if (n < 0) {
      std::cerr << "Error: cannot initialize Adt2d with n < 0: " << n << std::endl;
      throw(-1);
    }

    initAdt2d(n,bbmin,bbmax);
  ...
  void initAdt2d(int n,const T (*bbmin)[2],const T (*bbmax)[2]);

  void getBbox(T bbmin[2],T bbmax[2]) {
    // the global bounding box for all adt leaves is
    // stored in the first leaf (by construction)
    assert(leafdata);
    assert(nleaves > 0);
    FOR_I2 bbmin[i] = leafdata[0].bbmin[i];
    FOR_I2 bbmax[i] = leafdata[0].bbmax[i];
  }
  ...
  void buildListForSphericalShell(std::vector<int>& bboxVec,const T xs[2],const T r0,const T r1); // greater than r0 and less than or equal to r1
  ...
  void buildListForClosestPoint(std::vector<int>& bboxVec,const T point[2],double d2_max = 1.0E+20);
  ...
  void dumpLeafData(const int ileaf) {
    ...
    if (n == 0) {
      std::cout << "no leaf data available on this instance of Adt2d " << std::endl;
      return;
    }
  ...
  int get_level() const { return nstack;}
  int get_nleaves() const { return nleaves;}

  double get_delta() const {
    // returns the maximum bbox length scale...
    assert(nleaves >= 1);
    return max(leafdata[0].bbmax[0]-leafdata[0].bbmin[0],leafdata[0].bbmax[1]-leafdata[0].bbmin[1]);
  }

};

template <class T>
void Adt2d<T>::initAdt2d(int n,const T (*bbmin)[2],const T (*bbmax)[2]) {
  ...
  //cout << "Adt2d n = " << n << " nstack = " << nstack << " nleaves = " << nleaves << std::endl;
  ...
  /*
    std::cout << "Adt2d bbox = " << leafdata[ileaf].bbmin[0] << " < i < " << leafdata[ileaf].bbmax[0] << " , " <<
    leafdata[ileaf].bbmin[1] << " < j < " << leafdata[ileaf].bbmax[1] << std::endl;
  */
  ...
  if (n > 1) {
    ...
    while (istack > 0) {
      ...
      */
      ...
      std::sort(ii_f,ii_l,Adt2dIndexCompare<T>(bb,id));
      
      // used to be...
      //std::sort(&(index[i_f]),&(index[i_l+1]),Adt2dIndexCompare<T>(bb,id));
      
      // then chenged to (to avoid seg faults)...
      //std::sort(&(index[i_f]),&(index[i_l]),Adt2dIndexCompare<T>(bb,id));
  ...
  //std::cout << "done Adt2d()" << std::endl;

}

template <class T>
void Adt2d<T>::buildListForBBox(int &n_bbox_list,int * bbox_list,const T bbmin[2],const T bbmax[2]) {
  ...
  if (n > 0) {
    ...
    while (istack > 0) {
      ...
      if (leafdata[ileaf].i_f == leafdata[ileaf].i_l) {
        // add this index...
        // check...
        if (n_bbox_list >= ADT2D_LIST_MAX) {
          std::cerr << "Error: increase ADT2D_LIST_MAX: " << ADT2D_LIST_MAX << std::endl;
          throw(-1);
        }
...
template <class T>
void Adt2d<T>::buildListForBBox(std::vector<int>& bboxVec,const T bbmin[2],const T bbmax[2]) {
...
template <class T>
void Adt2d<T>::buildListForSphere(std::vector<int>& bboxVec,const T xs[2],const T rs) {
...
template <class T>
void Adt2d<T>::buildListForSphericalShell(std::vector<int>& bboxVec,const T xs[2],const T r0,const T r1) {
  ...
  if (n > 0) {
    ...
    while (istack > 0) {
      ...
      if ( ( calcMinD2PointToBbox(xs,leafdata[ileaf].bbmin,leafdata[ileaf].bbmax) > r1_2 ) || 
	   ( calcMaxD2PointToBbox(xs,leafdata[ileaf].bbmin,leafdata[ileaf].bbmax) <= r0_2 ) )
        continue;
...
template <class T>
void Adt2d<T>::buildListForPoint(int &n_bbox_list,int * bbox_list,const T point[2]) {
...
template <class T>
void Adt2d<T>::buildListForPoint(std::vector<int>& bboxVec,const T point[2]) {
...
template <class T>
void Adt2d<T>::buildListForClosestPoint(std::vector<int>& bboxVec,const T point[2],double d2_max) {

  // this routine allows the calling process to pass in d2_max, which can avoid 
  // traversing large parts (or all) of the tree when looking for the potentially
  // closest candidates...
  ...
  if (n > 0) {
    ...
    // if the closest corner of the global bbox is closer than d2_max, then it 
    // is possible that there are some "closest" points...
    T d2 = calcMinD2PointToBbox(point,leafdata[0].bbmin,leafdata[0].bbmax);
    if (d2 <= d2_max) {
      
      // each leaf stores its index and d2_min...
      std::list< std::pair<int,T> > leafList;
      leafList.push_back( std::pair<int,T>(0,d2) );

      // initialize d2_max with the full bounding box...
      d2 = calcMaxD2PointToBbox(point,leafdata[0].bbmin,leafdata[0].bbmax);
      d2_max = min(d2_max,d2);
