...
public:
  ...
  double x_sum[3];
  int count;
  LoopData(const set<int>& newSet,const double new_length,const double new_x[3]) {
    intSet = newSet;
    length_min = length_max = length_sum = new_length;
    FOR_I3 x_sum[i] = new_x[i];
    count = 1;
  }
  bool add(const set<int>& newSet,const double new_length,const double new_x[3]) {
    if (newSet.size() != intSet.size()) {
      return false;
    }
    set<int>::iterator it = intSet.begin();
    set<int>::iterator it_new = newSet.begin();
    while (it != intSet.end()) {
      assert(it_new != newSet.end());
      if (*it != *it_new) {
	return false;
      }
      ++it;
    ...
    // allow variations in length of 25%...
    const double length_avg = length_sum/double(count);
    if ((new_length > 1.25*length_avg)||(new_length < 0.75*length_avg)) {
      return false;
    }
    // looks like we can add...
    length_sum += new_length;
    length_min = min(length_min,new_length);
    length_max = max(length_max,new_length);
    FOR_I3 x_sum[i] += new_x[i];
    ++count;
    return true;
  }
...
void SimpleSurface::suggestMultiFluxProbeFlaggedZones() {

  // we need teost to do edge walking...

  ensureTeost();

  // now figure out which zones touch the flagged zones...
  
  cout << "building loops..." << endl;
  ...
  for (int ist = 0; ist < nst; ++ist) {
	...
	assert(b_nbr);
	assert(ist_nbr != ist);
	if (zoneVec[znost[ist_nbr]].flag != 1) {
	  // this edge is between the flagged zone of interest and an unflagged zone...
	  assert(sp_flag[spost[ist][i]] == -1);
	  sp_flag[spost[ist][i]] = ((ist<<2)|i); // teost-like so we can recover the zone info... 
	  assert(sp_flag[spost[ist][i]] >= 0);
	}
      }
    }
  }

  vector<LoopData> loopDataVec;
  for (int isp = 0; isp < nsp; ++isp) {
    if (sp_flag[isp] >= 0) {
      // start to walk these edges and see where they lead...
      int i = (sp_flag[isp]&3);
      int ist = (sp_flag[isp]>>2);
      assert(spost[ist][i] == isp);
      int ist_nbr;
      bool b_nbr = getAlignedTriNbr(ist_nbr,ist,i);
      assert(b_nbr);
      set<int> zoneSet;
      zoneSet.insert(znost[ist_nbr]);
      sp_flag[isp] = -2;
      int isp_start = isp;
      int isp_next = spost[ist][(i+1)%3];
      double wsum = DIST(xsp[isp_start],xsp[isp_next]);
      double xsum[3]; FOR_I3 xsum[i] = wsum*(xsp[isp_start][i]+xsp[isp_next][i]);
      while (isp_next != isp_start) {
	assert(sp_flag[isp_next] >= 0);
	i = (sp_flag[isp_next]&3);
	ist = (sp_flag[isp_next]>>2);
	assert(spost[ist][i] == isp_next);
	b_nbr = getAlignedTriNbr(ist_nbr,ist,i);
	assert(b_nbr);
	zoneSet.insert(znost[ist_nbr]);
	sp_flag[isp_next] = -2;
	const int isp_prev = isp_next;
	isp_next = spost[ist][(i+1)%3];
	const double w = DIST(xsp[isp_prev],xsp[isp_next]);
	wsum += w;
	FOR_I3 xsum[i] += w*(xsp[isp_prev][i]+xsp[isp_next][i]);
      }
      FOR_I3 xsum[i] /= 2.0*wsum;
      const int nii = loopDataVec.size();
      // try and add the loop data to the loopDataVec...
      int ii;
      for (ii = 0; ii < nii; ++ii) {
	if (loopDataVec[ii].add(zoneSet,wsum,xsum))
	  break;
      }
      // if we did not find anything close, then start a new one...
      if (ii == nii) {
	loopDataVec.push_back(LoopData(zoneSet,wsum,xsum));
      }
    }
  }
  ...
  for (int ii = 0, nii = loopDataVec.size(); ii < nii; ++ii) {
    cout << " > " << loopDataVec[ii].count << " loops with zones ";
    for (set<int>::iterator it = loopDataVec[ii].intSet.begin(); it != loopDataVec[ii].intSet.end(); ++it) {
      if (it != loopDataVec[ii].intSet.begin()) cout << ",";
      cout << zoneVec[*it].getName();
    }
    cout << " length (min,avg,max): " << loopDataVec[ii].length_min << " " << 
      loopDataVec[ii].length_sum/double(loopDataVec[ii].count) << " " <<
      loopDataVec[ii].length_max << " " << endl;
  }
