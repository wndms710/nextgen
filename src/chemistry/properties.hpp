// ==========================================================================================================
// classes and definitions for thermodynamics properties calculation of species and mixture.
...
#define AWT_S         32.065             ///< Atomic weights of S  atom in [kg/(kmol)] used in Species::ComputeSpeciesComposition().
...
typedef struct
{
  unsigned char C, H, N, O, BR, F, AR, CL, HE, S;
} compo;
...
{
  unsigned char C, H, N, O, BR, F, AR, HE, S;
} compo_old;
...
private:
  ...
  void ComputeSpeciesComposition()
  {
    ...
    cPtr->C = (unsigned char) 0, cPtr->H = (unsigned char) 0, 
      ...
      cPtr->CL = (unsigned char) 0, cPtr->S = (unsigned char) 0;

    if ((isoPtr = strrchr(ptr, '-'))||(isoPtr = strrchr(ptr, '_')))
      {
	ptr = &isoPtr[1];
      }
    while (*ptr)
	...
	if (theChar == 'A')
	  {
		...
		cPtr->C = cPtr->H = cPtr->N = cPtr->O = cPtr->F = cPtr->BR
		  = cPtr->AR = cPtr->HE = cPtr->CL = cPtr->S = 0;
	...
	if (theChar == 'H')
	  {
	    ...
	    else if ((*ptr != 'C') && (*ptr != 'N') && (*ptr != 'F') && (*ptr!= 'A') && (*ptr != 'H') && (*ptr != 'O') && (*ptr != 'B') && (*ptr != 'S')
    ...
    cout << "      C: " << int(cPtr->C ) << endl;
    cout << "      H: " << int(cPtr->H ) << endl;
    cout << "      O: " << int(cPtr->O ) << endl;
    cout << "      N: " << int(cPtr->N ) << endl;
    cout << "      F: " << int(cPtr->F ) << endl;
    cout << "      S: " << int(cPtr->S ) << endl;
    cout << "     BR: " << int(cPtr->BR) << endl;
    cout << "     CL: " << int(cPtr->CL) << endl;
    cout << "     AR: " << int(cPtr->AR) << endl;
    cout << "     HE: " << int(cPtr->HE) << endl << endl;
    */
  ...
  void ComputeSpeciesMolarMass()
  {
    M = (double) compo_elems.C * AWT_C + (double) compo_elems.H * AWT_H
      ...
      + (double) compo_elems.HE * AWT_HE + (double) compo_elems.S * AWT_S;
    return;
  }
...
public:
   ...
   */
  void ComputeSpeciesConstants(const double& Lewis_number)
  {
  ...
  void OutputSpeciesConstants() const
  {
    ...
    cout << "C=" << (int) compo_elems.C << "\t H=" << (int) compo_elems.H
	 ...
	 << "\t CL=" << (int) compo_elems.CL << "\t S=" << (int) compo_elems.S << endl;
...
public:
  ...
  void init(const string& filename, const vector<string>& speciesVec, const double Lewis_number){
    
    readThermoFileAndInitSpecies(filename,speciesVec,Lewis_number);
    
  }
  
  // overwrite the default copy constructor so that 
  // if the array is copied, it can be deconstructred 
  // properly 
  Mixture(const Mixture& other) {
  
    mySpecies            = other.mySpecies;  
    itSp                 = other.itSp;
    retS                 = other.retS;
    
    NkSpecies            = other.NkSpecies;
    T                    = other.T;
    M                    = other.M;
    R_over_M             = other.R_over_M;
    mul                  = other.mul;
    lambda               = other.lambda;
    gama                 = other.gama;
    ...
    dominant_species     = other.dominant_species;
    CombustionRegime     = other.CombustionRegime;
    OutputToScreen       = other.OutputToScreen;

    SpeciesName          = new char[NkSpecies][kNameLen];
    for (int i=0; i<NkSpecies; ++i)
      for (int j=0; j<kNameLen; ++j)
	SpeciesName[i][j] = other.SpeciesName[i][j];
  ...
  void Load()
  {
    ...
    NkSpecies = getIntParam("NUMBER_SPECIES");
    if (CombustionRegime > 0) dominant_species = getStringParam("DOM_SPECIES");
    SpeciesName = new char[NkSpecies][kNameLen];
    Yk_left  = new double[NkSpecies];
    Yk_right = new double[NkSpecies];

    if (CombustionRegime == -1)
      bc_l = getParam("MIXTURE_COMPO");
    else if (CombustionRegime == 0)
      {
	bc_l = getParam("MIXING_BC_LEFT");
	bc_r = getParam("MIXING_BC_RIGHT");
      } 
    p = getParam("SPECIES");
    if (NkSpecies != p->size())
      {
	cerr << "### Number of species read " << p->size() << " does not match the input number of species " << NkSpecies << " ###" << endl;
	throw(-1);
      }
    for (i = 0; i < NkSpecies; i++)
      {
	buffer_str = p->getString(i);
    ...
    buffer_str = getStringParam("THERMO_INPUT_FILE");
    ...
    // DBG    0 ???? or "0"
    thermo_blanq_flag = getIntParam("THERMO_BLANQ_FLAG", 0);
	...
	dum = fread(&int_buff, sizeof(int_buff), 1, fp);
	if (dum != 1)
	  {
	...
	while (fread(&sr, sizeof(SR_buff_blanquart), 1, fp))
	  {
	...
	/* Loop over input file, read data and create/load new species if required */
	while (fread(&srfm, sizeof(SR_buff_FlameMaster), 1, fp))
	  {
	    /* Check if species should be loaded by comparing names */
	    for (i = 0; i < NkSpecies; i++)
	      {
    ...
    for (itSp = mySpecies.begin(); itSp != mySpecies.end(); itSp++)
      {
	itSp->second.ComputeSpeciesConstants(Lewis);
      }
  ...
  void Load(int CombReg, string output = "YES")
  {
...
private:
  ...
  // =======================================================================================================
  // load the thermo file
  // The species are stored in a map container with their name (string) as key to retrieve them easily.
  // Thermodynamics input file (binary) is the same as the input file for ScanMan in FlameMaster, created by
  // FlameMaster tool \p CreateBinFile.
  // warning The exact format of the binary thermodynamics input file must match!
  // ======================================================================================================== 
  void readThermoFileAndInitSpecies(const string& filename, const vector<string>& speciesVec, const double Lewis_number){
    
    // initialize number of species
    NkSpecies = speciesVec.size();


    // use the species flag to check if a species is read
    int* species_flag = new int[NkSpecies];
    // initialize
    for (int i=0; i<NkSpecies; ++i) species_flag[i] = 0;
    
    assert(SpeciesName == NULL);
    SpeciesName = new char[NkSpecies][kNameLen];
    
    for (int i=0; i<NkSpecies; ++i){
      char buffer_m[kNameLen];
      for (int j=0; j<kNameLen; ++j) {
	SpeciesName[i][j] = '\0';
	buffer_m[j] = '\0';
      }
      string buffer_str = speciesVec[i];
      size_t dum = buffer_str.copy(&buffer_m[0], kNameLen);
      dum = buffer_str.size();
      //if (dum < kNameLen)
      //	buffer_m[dum] = '\0';
      strcpy(&SpeciesName[i][0], buffer_m);
    ...
    cout << "     > reading thermo file: " << filename << endl;
    FILE * fp = NULL;
    fp = fopen(filename.c_str(),"rb");
    if ( !fp ) {
      cerr << ERRSTART << "Error: could not open file " << filename << ERREND << endl;
      throw(0);
    }
    ...
    while ( true ) {
      SR_buff_FlameMaster srfm;
      size_t size_read = fread(&srfm,sizeof(SR_buff_FlameMaster),1,fp);
      if ( size_read != 1 )
	break;
      /// Check if species should be loaded by comparing names 
      for (int i=0; i<NkSpecies; ++i) {
	...
	if (!strcmp(buffer_m, srfm.name)) {
	  ...
	  retS = mySpecies.insert(pair<string, Species> (string(key),Species()));
	  if (retS.second == false){
	    cerr << ERRSTART << "  > species " << key << " already in mySpecies (in loop i=" << i << ")" << ERREND << endl;
	    throw(0);
	  }
	  else {
	    /* Populate new species */
	    ...
	    retS.first->second.SetSpeciesAlpha(-1.0);
	    retS.first->second.SetSpeciesZrot(-1.0);
	    retS.first->second.SetYk_bc_left(-1);
	    retS.first->second.SetYk_bc_right(-1);
	  }
    ...
    /* Check if all species given by input file initialized */
    if (count != NkSpecies) {
      cerr << " > could not find the following species required by the flamelets in the thermo file " << filename << endl;
      for (int i=0; i<NkSpecies; ++i)
	if ( species_flag[i] == 0 )
	  cerr << "    " << speciesVec[i] << endl;
      cerr << ERRSTART << "Error: number of species loaded from thermo file (" << count << 
	") does not match the input number of species in the flamelet file("<<NkSpecies <<")" << ERREND << endl;
      throw(0);
    }
    delete [] species_flag;
    
      // Now initialize some species constants 
    // Lewis number: take it as an input parameter
    Lewis = Lewis_number; 
    for (itSp = mySpecies.begin(); itSp != mySpecies.end(); itSp++)
      itSp->second.ComputeSpeciesConstants(Lewis);
